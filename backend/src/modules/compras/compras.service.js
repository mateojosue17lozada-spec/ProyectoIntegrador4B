...
exports.cancelar=async(id,usuario,req)=>{const r=await pool.query("UPDATE ordenes_compra SET estado='Cancelada' WHERE id_orden_compra=$1 AND estado IN ('Pendiente','Aprobada') RETURNING *",[id]);if(!r.rows[0])throw Object.assign(new Error("Orden no disponible para cancelar"),{status:409});await registrarAuditoria({idUsuario:usuario.id,accion:"ORDEN_COMPRA_CANCELADA",tabla:"ordenes_compra",registroId:Number(id),req});return r.rows[0]};
...
exports.recibir=async(id,data,usuario,req)=>{const client=await pool.connect();try{await client.query("BEGIN");
 const orden=await client.query("SELECT * FROM ordenes_compra WHERE id_orden_compra=$1 FOR UPDATE",[id]);if(!orden.rows[0])throw new Error("Orden no encontrada");if(!["Pendiente","Aprobada"].includes(orden.rows[0].estado))throw Object.assign(new Error("La orden no está disponible para recepción"),{status:409});
 const proveedor=await client.query("SELECT dias_credito FROM proveedores WHERE id_proveedor=$1",[orden.rows[0].id_proveedor]);
 ...
 for(const d of detalles.rows){if(!d.id_producto)continue;await client.query(`UPDATE productos SET costo=CASE WHEN stock+$1>0 THEN ((stock*costo)+($1*$2))/(stock+$1) ELSE $2 END,stock=stock+$1 WHERE id_producto=$3`,[d.cantidad,d.costo_unitario,d.id_producto]);await client.query("INSERT INTO recepcion_compra_detalle(id_recepcion,id_producto,cantidad,costo_unitario) VALUES($1,$2,$3,$4)",[rec.rows[0].id_recepcion,d.id_producto,d.cantidad,d.costo_unitario])}
 ...
 VALUES($1,$2,$3,CURRENT_DATE+$4::integer)`,[orden.rows[0].id_proveedor,id,orden.rows[0].total,Number(data.dias_credito ?? proveedor.rows[0]?.dias_credito ?? 0)]);
